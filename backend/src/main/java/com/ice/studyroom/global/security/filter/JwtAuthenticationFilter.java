...
public class JwtAuthenticationFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws
		...
		if (token != null) {
			try {
				jwtTokenProvider.validateToken(token, isRefreshTokenRequest); // 유효성 검사에서 예외 발생 시 catch 로 이동
				...
				if (isRefreshTokenRequest) {
					...
					// Refresh API 는 만료된 AT를 허용하고 내부 로직에서 재발급
					chain.doFilter(request, response);
					return;
				}

				// 일반 요청인데 만료된 AT 라면 401 Unauthorized 응답, 프론트는 위의 조건문으로 전달될 수 있는 요청할 예정
				...
				// ExceptionTranslationFilter 의 도움으로 예외 핸들러로 전달 가능
