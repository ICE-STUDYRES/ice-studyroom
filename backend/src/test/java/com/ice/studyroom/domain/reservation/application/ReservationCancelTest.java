...
class ReservationCancelTest {
	...
	void 예약_1시간_취소_성공() {
		...
		verify(scheduleCanceller).cancelAssociatedSchedules(reservationId, firstScheduleId, Optional.empty());
	 ...
	 */
	@Test
	@DisplayName("입실 1시간 전 2시간 예약 취소 성공 (패널티 없음)")
	void 예약_2시간_취소_성공() {
		// given
		기본_예약_정보_셋업(token, reservationId, userEmail);
		시간_고정_셋업(12, 30);
		...
		verify(reservation).cancel(any(LocalDateTime.class));
		verify(scheduleCanceller).cancelAssociatedSchedules(reservationId, firstScheduleId, Optional.of(secondScheduleId));
		verify(penaltyService, never()).assignPenalty(any(), anyLong(), any());
	}
	...
	void 입실_시간_이후_취소는_불가능_하다는_예외_발생() {
		...
		verify(scheduleCanceller, never()).cancelAssociatedSchedules(anyLong(), anyLong(), any());
	}
	...
	void 한시간_예약_스케줄_셋업() {
		given(reservation.getFirstScheduleId()).willReturn(firstScheduleId);
		given(reservation.getSecondScheduleIdOpt()).willReturn(Optional.empty());
	}

	void 두시간_예약_스케줄_셋업() {
		given(reservation.getFirstScheduleId()).willReturn(firstScheduleId);
		given(reservation.getSecondScheduleIdOpt()).willReturn(Optional.of(secondScheduleId));
	}
}
